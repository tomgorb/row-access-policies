...
END IF
 ;
EXECUTE IMMEDIATE
 "CREATE OR REPLACE TABLE `{PROJECT_ID}.{TARGET_DATASET}.{TABLE_ID}_part_" || i || "`LIKE `{PROJECT_ID}.{TARGET_DATASET}.{TABLE_ID}`";
SET
 (identifiers,
   farms) = (
 SELECT
...
CREATE SCHEMA IF NOT EXISTS `{PROJECT_ID_VIEW}.{VIEWS_DATASET}` OPTIONS (location = 'EU');
LOOP
SET
 j = j + 1;
IF
...
END LOOP
 ;
EXECUTE IMMEDIATE
 "CREATE OR REPLACE VIEW `{PROJECT_ID_VIEW}.{VIEWS_DATASET}.{VIEW_NAME}` AS (" || QUERIES || ")";
